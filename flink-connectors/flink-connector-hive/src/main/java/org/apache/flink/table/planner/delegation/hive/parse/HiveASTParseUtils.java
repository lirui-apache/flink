...
public class HiveASTParseUtils {
	 ...
	 */
	public static ASTNode parse(String command, HiveParserContext ctx) throws HiveASTParseException {
	...
	public static ASTNode parse(
			String command, HiveParserContext ctx, String viewFullyQualifiedName) throws HiveASTParseException {
	...
	private static void processSetColsNode(ASTNode setCols, ASTSearcher searcher) {
		...
		ASTNode select = searcher.simpleBreadthFirstSearchAny((ASTNode) fromNode,
				HiveASTParser.TOK_SELECT, HiveASTParser.TOK_SELECTDI);
		if (select == null) {
			// Couldn't find the from that contains subquery; replace with ALLCOLREF.
	...
	private static boolean createChildColumnRef(Tree child, String alias,
		...
		HiveParserASTBuilder selExpr = HiveParserASTBuilder.construct(HiveASTParser.TOK_SELEXPR, "TOK_SELEXPR");
		HiveParserASTBuilder toc = HiveParserASTBuilder.construct(HiveASTParser.TOK_TABLE_OR_COL, "TOK_TABLE_OR_COL");
		HiveParserASTBuilder id = HiveParserASTBuilder.construct(HiveASTParser.Identifier, colAlias);
		if (alias == null) {
			selExpr = selExpr.add(toc.add(id));
		} else {
			HiveParserASTBuilder dot = HiveParserASTBuilder.construct(HiveASTParser.DOT, ".");
			HiveParserASTBuilder aliasNode = HiveParserASTBuilder.construct(HiveASTParser.Identifier, alias);
	...
	public static ExprNodeDesc createConversionCast(ExprNodeDesc column, PrimitiveTypeInfo tableFieldTypeInfo)
		...
		return HiveParserTypeCheckProcFactory.DefaultExprProcessor.getFuncExprNodeDescWithUdfData(baseType,
