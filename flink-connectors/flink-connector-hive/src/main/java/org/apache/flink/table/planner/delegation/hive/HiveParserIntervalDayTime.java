	...
	public static HiveParserIntervalDayTime valueOf(String strVal) {
		...
		if (patternMatcher.matches()) {
			...
			try {
				...
				int days = sign *
						HiveParserIntervalUtils.parseNumericValueWithRange("day", patternMatcher.group(2),
								0, Integer.MAX_VALUE);
				byte hours = (byte) (sign *
						HiveParserIntervalUtils.parseNumericValueWithRange("hour", patternMatcher.group(3), 0, 23));
				byte minutes = (byte) (sign *
						HiveParserIntervalUtils.parseNumericValueWithRange("minute", patternMatcher.group(4), 0, 59));
				int seconds = 0;
				int nanos = 0;
				...
				if (field != null) {
					...
					nanos = sign * bdSeconds.subtract(new BigDecimal(bdSeconds.toBigInteger()))
							.multiply(HiveParserIntervalUtils.NANOS_PER_SEC_BD).intValue();
				}
