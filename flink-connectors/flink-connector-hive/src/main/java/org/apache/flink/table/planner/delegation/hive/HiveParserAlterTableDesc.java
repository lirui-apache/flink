...
public class HiveParserAlterTableDesc implements Serializable {
	...
	private HiveParserAlterTableDesc(AlterTableDesc.AlterTableTypes op, String compoundName, Map<String, String> partSpec,
			boolean expectView, Map<String, String> props, String newName, String serdeName, String newLocation, String oldColName,
			String newColName, String newColType, String newColComment, boolean first, String after,
			List<FieldSchema> newCols, boolean cascade) {
		this.op = op;
	...
	public static HiveParserAlterTableDesc alterFileFormat(String compoundName, Map<String, String> partSpec) {
		return new Builder().op(ADDFILEFORMAT).compoundName(compoundName).partSpec(partSpec).build();
	}

	public static HiveParserAlterTableDesc changeColumn(String compoundName, String oldColName, String newColName,
			String newColType, String newColComment, boolean first, String after, boolean cascade) {
		return new Builder().op(RENAMECOLUMN).compoundName(compoundName).oldColName(oldColName).newColName(newColName)
				.newColType(newColType).newColComment(newColComment).first(first).after(after).cascade(cascade).build();
	}

	public static HiveParserAlterTableDesc addReplaceColumns(String compoundName, List<FieldSchema> newCols, boolean replace, boolean cascade) {
		return new Builder().op(replace ? REPLACECOLS : ADDCOLS).compoundName(compoundName).newCols(newCols).cascade(cascade).build();
	}

	public static HiveParserAlterTableDesc rename(String compoundName, String newName, boolean expectView) {
		return new Builder().op(RENAME).compoundName(compoundName).newName(newName).expectView(expectView).build();
	}

	public static HiveParserAlterTableDesc alterTableProps(String compoundName, Map<String, String> partSpec,
			Map<String, String> props, boolean expectView) {
		return new Builder().op(ADDPROPS).compoundName(compoundName).partSpec(partSpec).props(props).expectView(expectView).build();
	}

	public static HiveParserAlterTableDesc alterSerDe(String compoundName, Map<String, String> partSpec, String serdeName,
			Map<String, String> props) {
		return new Builder().op(serdeName == null ? ADDSERDEPROPS : ADDSERDE).compoundName(compoundName).partSpec(partSpec)
				.serdeName(serdeName).props(props).build();
	}

	public static HiveParserAlterTableDesc alterLocation(String compoundName, Map<String, String> partSpec, String newLocation) {
		return new Builder().op(ALTERLOCATION).compoundName(compoundName).partSpec(partSpec).newLocation(newLocation).build();
	}
	...
	private static class Builder {
		...
		HiveParserAlterTableDesc build() {
			return new HiveParserAlterTableDesc(op, compoundName, partSpec, expectView, props, newName, serdeName,
					newLocation, oldColName, newColName, newColType, newColComment, first, after, newCols, cascade);
		}
	}
}
