	...
	public boolean fillStorageFormat(ASTNode child) throws SemanticException {
		switch (child.getToken().getType()) {
			case HiveASTParser.TOK_TABLEFILEFORMAT:
				...
				inputFormat = HiveParserBaseSemanticAnalyzer.unescapeSQLString(child.getChild(0).getText());
				outputFormat = HiveParserBaseSemanticAnalyzer.unescapeSQLString(child.getChild(1).getText());
				if (child.getChildCount() == 3) {
					serde = HiveParserBaseSemanticAnalyzer.unescapeSQLString(child.getChild(2).getText());
				}
				break;
			case HiveASTParser.TOK_STORAGEHANDLER:
				storageHandler = HiveParserBaseSemanticAnalyzer.unescapeSQLString(child.getChild(0).getText());
				if (child.getChildCount() == 2) {
	...
	protected void fillDefaultStorageFormat(boolean isExternal, boolean isMaterializedView)
		...
		if ((inputFormat == null) && (storageHandler == null)) {
			...
			if (isMaterializedView) {
				defaultFormat = defaultManagedFormat = conf.get("hive.materializedview.fileformat", "ORC");
