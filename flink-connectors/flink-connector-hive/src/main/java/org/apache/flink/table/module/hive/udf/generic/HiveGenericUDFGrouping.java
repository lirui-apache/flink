...
/**
 * Hive's GROUPING function relies on the GROUPING__ID value. In earlier versions the GROUPING function requires
 * the GROUPING__ID to be an integer while in Flink the value is a long. Therefore we copy the GROUPING function from
 * newer version and HiveModule should always return this one.
...
public class HiveGenericUDFGrouping extends GenericUDF {
	...
	public ObjectInspector initialize(ObjectInspector[] arguments) throws UDFArgumentException {
		...
		if (!(arg1OI.getPrimitiveCategory() == PrimitiveObjectInspector.PrimitiveCategory.INT ||
				arg1OI.getPrimitiveCategory() == PrimitiveObjectInspector.PrimitiveCategory.LONG)) {
