...
import static org.apache.flink.table.planner.delegation.hive.parse.HiveParserBaseSemanticAnalyzer.escapeSQLString;
...
public class HiveParserASTBuilder {
	...
	public static ASTNode destNode() {
		return HiveParserASTBuilder
				.construct(HiveASTParser.TOK_DESTINATION, "TOK_DESTINATION")
				.add(
						HiveParserASTBuilder.construct(HiveASTParser.TOK_DIR, "TOK_DIR").add(HiveASTParser.TOK_TMP_FILE,
								"TOK_TMP_FILE")).node();
	}
	...
	public static ASTNode join(ASTNode left, ASTNode right, JoinRelType joinType, ASTNode cond,
		...
		switch (joinType) {
			case INNER:
				if (semiJoin) {
					b = HiveParserASTBuilder.construct(HiveASTParser.TOK_LEFTSEMIJOIN, "TOK_LEFTSEMIJOIN");
				} else {
					b = HiveParserASTBuilder.construct(HiveASTParser.TOK_JOIN, "TOK_JOIN");
				}
				break;
			case LEFT:
				b = HiveParserASTBuilder.construct(HiveASTParser.TOK_LEFTOUTERJOIN, "TOK_LEFTOUTERJOIN");
				break;
			case RIGHT:
				b = HiveParserASTBuilder.construct(HiveASTParser.TOK_RIGHTOUTERJOIN, "TOK_RIGHTOUTERJOIN");
				break;
			case FULL:
				b = HiveParserASTBuilder.construct(HiveASTParser.TOK_FULLOUTERJOIN, "TOK_FULLOUTERJOIN");
				break;
		}
	...
	public static ASTNode subQuery(ASTNode qry, String alias) {
		return HiveParserASTBuilder.construct(HiveASTParser.TOK_SUBQUERY, "TOK_SUBQUERY").add(qry)
				.add(HiveASTParser.Identifier, alias).node();
	}

	public static ASTNode qualifiedName(String tableName, String colName) {
		HiveParserASTBuilder b = HiveParserASTBuilder
				.construct(HiveASTParser.DOT, ".")
				.add(
						HiveParserASTBuilder.construct(HiveASTParser.TOK_TABLE_OR_COL, "TOK_TABLE_OR_COL").add(
								HiveASTParser.Identifier, tableName)).add(HiveASTParser.Identifier, colName);
		return b.node();
	}

	public static ASTNode unqualifiedName(String colName) {
		HiveParserASTBuilder b = HiveParserASTBuilder.construct(HiveASTParser.TOK_TABLE_OR_COL, "TOK_TABLE_OR_COL").add(
				HiveASTParser.Identifier, colName);
		return b.node();
	}

	public static ASTNode where(ASTNode cond) {
		return HiveParserASTBuilder.construct(HiveASTParser.TOK_WHERE, "TOK_WHERE").add(cond).node();
	}

	public static ASTNode having(ASTNode cond) {
		return HiveParserASTBuilder.construct(HiveASTParser.TOK_HAVING, "TOK_HAVING").add(cond).node();
	}

	public static ASTNode limit(Object offset, Object limit) {
		return HiveParserASTBuilder.construct(HiveASTParser.TOK_LIMIT, "TOK_LIMIT")
				.add(HiveASTParser.Number, offset.toString())
				.add(HiveASTParser.Number, limit.toString()).node();
	}

	public static ASTNode selectExpr(ASTNode expr, String alias) {
		return HiveParserASTBuilder.construct(HiveASTParser.TOK_SELEXPR, "TOK_SELEXPR").add(expr)
				.add(HiveASTParser.Identifier, alias).node();
	}
	...
	public static ASTNode literal(RexLiteral literal, boolean useTypeQualInLiteral) {
		...
		switch (sqlType) {
			...
			case INTERVAL_YEAR_MONTH:
				if (literal.getValue() == null) {
					return HiveParserASTBuilder.construct(HiveASTParser.TOK_NULL, "TOK_NULL").node();
				}
				break;
			case TINYINT:
			case SMALLINT:
		...
		switch (sqlType) {
			case TINYINT:
				...
				type = HiveASTParser.IntegralLiteral;
				break;
			case SMALLINT:
				if (useTypeQualInLiteral) {
			...
			case DOUBLE:
				...
				type = HiveASTParser.NumberLiteral;
				break;
			case DECIMAL:
			...
			case REAL:
				val = literal.getValue3();
				type = HiveASTParser.Number;
				break;
			case VARCHAR:
			case CHAR:
				val = literal.getValue3();
				String escapedVal = escapeSQLString(String.valueOf(val));
				type = HiveASTParser.StringLiteral;
				val = "'" + escapedVal + "'";
				break;
			case BOOLEAN:
				val = literal.getValue3();
				type = ((Boolean) val).booleanValue() ? HiveASTParser.KW_TRUE : HiveASTParser.KW_FALSE;
				break;
			case DATE: {
				val = literal.getValue();
				type = HiveASTParser.TOK_DATELITERAL;
			...
			case TIMESTAMP: {
				val = literal.getValue();
				type = HiveASTParser.TOK_TIMESTAMPLITERAL;
			...
			case INTERVAL_MONTH:
			case INTERVAL_YEAR_MONTH: {
				type = HiveASTParser.TOK_INTERVAL_YEAR_MONTH_LITERAL;
			...
			case INTERVAL_MINUTE_SECOND:
			case INTERVAL_SECOND: {
				type = HiveASTParser.TOK_INTERVAL_DAY_TIME_LITERAL;
