...
LOAD DATA LOCAL INPATH '${hiveconf:test.data.dir}/schema_evolution/schema_evolution_data.txt' overwrite into table schema_evolution_data;

CREATE TABLE schema_evolution_data_2(insert_num int, boolean1 boolean, tinyint1 tinyint, smallint1 smallint, int1 int, bigint1 bigint, decimal1 decimal(38,18), float1 float, double1 double, string1 string, string2 string, date1 date, timestamp1 timestamp, boolean_str string, tinyint_str string, smallint_str string, int_str string, bigint_str string, decimal_str string, float_str string, double_str string, date_str string, timestamp_str string, filler string)
row format delimited fields terminated by '|' stored as textfile;
LOAD DATA LOCAL INPATH '${hiveconf:test.data.dir}/schema_evolution/schema_evolution_data_2.txt' overwrite into table schema_evolution_data_2;
...
stored as textfile;
LOAD DATA LOCAL INPATH '${hiveconf:test.data.dir}/schema_evolution/same_type1_a.txt' overwrite into table same_type1_a_txt;
...
row format delimited fields terminated by '|'
stored as textfile;
LOAD DATA LOCAL INPATH '${hiveconf:test.data.dir}/schema_evolution/same_type1_b.txt' overwrite into table same_type1_b_txt;
...
row format delimited fields terminated by '|'
stored as textfile;
LOAD DATA LOCAL INPATH '${hiveconf:test.data.dir}/schema_evolution/same_type1_c.txt' overwrite into table same_type1_c_txt;
...
drop table if exists schema_evolution_data;

drop table if exists same_type1_b_txt;

drop table if exists same_type1_c_txt;

drop table if exists same_type1_a_txt;

drop table if exists schema_evolution_data_2;
