...
LOAD DATA LOCAL INPATH "${hiveconf:test.data.dir}/customers.txt" INTO TABLE customers;

CREATE TABLE orders (id int, cid int, d date, amount double)
ROW FORMAT DELIMITED
FIELDS TERMINATED BY ',';

LOAD DATA LOCAL INPATH "${hiveconf:test.data.dir}/orders.txt" INTO TABLE orders;
...
LOAD DATA LOCAL INPATH "${hiveconf:test.data.dir}/nested_orders.txt" INTO TABLE nested_orders;
