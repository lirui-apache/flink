...
public class HiveDialectQueryITCase {
	...
	public static void setup() throws Exception {
		...
		hiveCatalog.open();
		tableEnv = getTableEnvWithHiveCatalog(SqlDialect.HIVE);
	...
	@Test
	public void testQueryCompatible() throws Exception {
		Assume.assumeTrue(HiveVersionTestUtil.HIVE_120_OR_LATER && !HiveVersionTestUtil.HIVE_310_OR_LATER);
...
//		runQuery("select x,count(y),max(y) from foo group by x", tableEnv);
	...
	private static TableEnvironment getTableEnvWithHiveCatalog(SqlDialect dialect) {
